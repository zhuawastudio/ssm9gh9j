<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="宣传片" prop="xuanchuanpian">
          <file-upload
          tip="点击上传宣传片"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.xuanchuanpian?ruleForm.xuanchuanpian:''"
          @change="xuanchuanpianUploadChange"
          ></file-upload>
        </el-form-item> 
        <div v-else>
          <el-form-item v-if="ruleForm.xuanchuanpian" label="宣传片" prop="xuanchuanpian">
            <el-button type="text" size="small" @click="download(ruleForm.xuanchuanpian)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.didian = true;
          ...
          if(o=='xuanchuanpian'){
            this.ruleForm.xuanchuanpian = obj[o];
	    this.ro.xuanchuanpian = true;
            continue;
          }
          if(o=='xiangqing'){
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../ssm9gh9j/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yanchanghui/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yanchanghuiCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    xuanchuanpianUploadChange(fileUrls) {
	this.ruleForm.xuanchuanpian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
